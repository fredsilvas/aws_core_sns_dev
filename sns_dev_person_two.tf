...
resource "aws_sns_topic" "sns_dev_person_two" {
  name         = "sns_dev_person_two"
  display_name = "SNS Dev - Person Two"
  ...
  provisioner "local-exec" {
    command = "aws sns subscribe --topic-arn ${self.arn} --protocol email --notification-endpoint ${var.sns_email_dev_person_two}"
    when    = create
  }
