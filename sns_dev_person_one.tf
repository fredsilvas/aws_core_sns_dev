...
resource "aws_sns_topic" "sns_dev_person_one" {
  name         = "sns_dev_person_one"
  display_name = "SNS Dev - Person One"
  ...
  provisioner "local-exec" {
    command = "aws sns subscribe --topic-arn ${self.arn} --protocol email --notification-endpoint ${var.sns_email_dev_person_one}"
    when    = create
  }
